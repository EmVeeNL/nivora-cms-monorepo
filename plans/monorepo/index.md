...
4. ✅ PR template — `.github/pull_request_template.md` (added in CI/CD addendum)
...
## Addendum — CI/CD (added 2026-06-17, feature/ci-setup PR #1)

Added after initial completion:
- `.github/workflows/ci.yml` — typecheck + lint + test in parallel on push/PR to main+develop
- `.github/workflows/deploy.yml` — Cloudflare Workers deploy on push to main; needs `CLOUDFLARE_API_TOKEN` in GitHub Environments → production
- `.github/pull_request_template.md`
- `.github/dependabot.yml` — weekly grouped updates targeting develop
- Branch protection on main (CI + PR required) and develop (CI required) via `gh api`
- Deploy script corrected: `wrangler deploy --config dist/server/wrangler.json` (generated by `vite build`; do not use source `wrangler.jsonc`)
- Tsconfig path cleanup: removed manual `@nivora-cms/*` path overrides; `moduleResolution: bundler` resolves through `package.json` exports via pnpm symlinks
- `packageManager: pnpm@10.33.0` added to root `package.json`
- `typecheck` scripts added to each package; root runs `pnpm -r run typecheck`
